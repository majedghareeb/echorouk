...
class PhpRedisObjectCache extends ObjectCache implements MeasuredObjectCacheInterface
{
    ...
    public function add($key, $data, string $group = 'default', int $expire = 0): bool
    {
        ...
        // FAIL-SAFE: Check for Multisite "Split Brain" (WAMS Patch)
        if ($this->isMultisite && $group === 'options' && $key === 'alloptions') {
            global $wpdb;

            if (isset($wpdb->blogid) && (int) $wpdb->blogid !== (int) $this->blogId) {
                // Log mismatch
                error_log("Object Cache Pro FAIL-SAFE: Blog ID mismatch detected during ADD. Cache: {$this->blogId}, DB: {$wpdb->blogid}. Correcting...");

                // Force sync
                $this->switch_to_blog((int) $wpdb->blogid);
            }
        }
        ...
        try {
            ...
            $result = (bool) $this->write($id, $data, $group, $expire, 'NX');
    ...
    {
        if ($group === 'options' && isset($data['alloptions'])) {
            // FAIL-SAFE: Check for Multisite "Split Brain" (WAMS Patch)
            if ($this->isMultisite) {
                global $wpdb;
                if (isset($wpdb->blogid) && (int) $wpdb->blogid !== (int) $this->blogId) {
                    error_log("Object Cache Pro FAIL-SAFE: Blog ID mismatch detected during ADD_MULTIPLE. Cache: {$this->blogId}, DB: {$wpdb->blogid}. Correcting...");
                    $this->switch_to_blog((int) $wpdb->blogid);
                }
            }
        }
    ...
    public function flush_group(string $group): bool
    {
        ...
        // FIX: The following block forces a global flush for non-global groups (like 'options'),
        // which causes massive cache regeneration storms and race conditions. Disabled by WAMS patch.
    ...
    public function replace($key, $data, string $group = 'default', int $expire = 0): bool
    {
        ...
        if ($this->isMultisite && $group === 'options' && $key === 'alloptions') {
            ...
            if (isset($wpdb->blogid) && (int) $wpdb->blogid !== (int) $this->blogId) {
                // Log mismatch
                error_log("Object Cache Pro FAIL-SAFE: Blog ID mismatch detected during REPLACE. Cache: {$this->blogId}, DB: {$wpdb->blogid}. Correcting...");
        ...
        try {
            $result = (bool) $this->write($id, $data, $group, $expire, 'XX');
    ...
    public function set($key, $data, string $group = 'default', int $expire = 0): bool
    {
        ...
        if ($this->isMultisite && $group === 'options' && $key === 'alloptions') {
            ...
            if (isset($wpdb->blogid) && (int) $wpdb->blogid !== (int) $this->blogId) {
                // Log mismatch
                error_log("Object Cache Pro FAIL-SAFE: Blog ID mismatch detected during write. Cache: {$this->blogId}, DB: {$wpdb->blogid}. Correcting...");
        ...
        try {
            ...
            $result = (bool) $this->write($id, $data, $group, $expire);
    ...
    public function set_multiple(array $data, string $group = 'default', int $expire = 0): array
    {
        if ($group === 'options' && isset($data['alloptions'])) {
            ...
            if ($this->isMultisite) {
                global $wpdb;
                if (isset($wpdb->blogid) && (int) $wpdb->blogid !== (int) $this->blogId) {
                    error_log("Object Cache Pro FAIL-SAFE: Blog ID mismatch detected during SET_MULTIPLE. Cache: {$this->blogId}, DB: {$wpdb->blogid}. Correcting...");
                    $this->switch_to_blog((int) $wpdb->blogid);
                }
            }
        }
    ...
    /**
     * Writes the given key to Redis and enforces the `maxttl` and `queryttl` options.
     *
     * @param  string  $id
     * @param  mixed  $data
     * @param  string  $group
     ...
     * @return bool
     */
    protected function write(string $id, $data, string $group, int $expire = 0, $option = null): bool
    {
        if ($expire < 0) {
            $expire = 0;
        }

        $maxttl = array_key_exists($group, $this->queryGroups)
            ? min($this->config->queryttl, $this->config->maxttl)
            : $this->config->maxttl;
    ...
    public function info()
    {
        ...
        $info->meta = array_filter([
            ...
            'Redis Memory' => size_format($server['used_memory'] ?? 0, 2),
