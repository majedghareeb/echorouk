...
class Diagnostics implements ArrayAccess
{
     ...
     *
     * @var array<string, array<int|string, Diagnostic>>
     */
    protected $data = [];
    ...
    protected function gatherGeneral()
    {
        ...
        $this->data[self::GENERAL]['server-type'] = $this->serverType();
    ...
    /**
     * Gathers information about Relay, if installed.
     *
     * @return void
     */
    protected function gatherRelay()
    {
        ...
        $this->data[self::RELAY]['relay-strategy'] = $this->relayStrategy();
        $this->data[self::RELAY]['relay-eviction'] = $this->relayEvictionPolicy();
        $this->data[self::RELAY]['relay-adaptive-load'] = $this->relayAdaptiveLoadFactor();
    ...
    protected function gatherRedisStatistics()
    {
        ...
        $this->data[self::STATISTICS]['redis-tracking'] = $this->keyTracking();
    }
    ...
    protected function gatherErrors()
    {
        ...
        if ($this->config->initException ?? false) {
            $this->data[self::ERRORS][] = Diagnostic::name('Initialization')->error(sprintf(
        ...
        foreach ($wp_object_cache_errors ?? [] as $error) {
            $this->data[self::ERRORS][] = Diagnostic::name('Error')->error($error);
        }
    }
    ...
    protected function relayCache()
    {
        ...
        if (! $this->clientIsRelay()) {
            return $diagnostic->warning('Inactive');
        }

        return $this->usingRelayCache()
            ? $diagnostic->success('Enabled')
            : $diagnostic->value('Disabled')->comment('client only');
    }

    /**
     * Returns Relay's caching strategy.
     *
     * @return \RedisCachePro\Diagnostics\Diagnostic
     */
    public function relayStrategy()
    {
        $diagnostic = Diagnostic::name('Relay Strategy');

        if (! $this->usingRelay()) {
            return $diagnostic->value('None');
        }

        if (! $this->usingRelayCache()) {
            return $diagnostic->value('Client');
        }

        if ($this->usingRelayAdaptiveCache()) {
            return $diagnostic->value('Adaptive');
        }

        $defaults = (new \ReflectionClass(Configuration::class))
            ->getDefaultProperties()['relay'] ?? [];

        if (
            $defaults['allowed'] !== $this->config->relay->allowed ||
            $defaults['ignored'] !== $this->config->relay->ignored
        ) {
            return $diagnostic->value('Selective');
        }

        return $diagnostic->value('Universal');
    }
     ...
     * @return \RedisCachePro\Diagnostics\Diagnostic
     */
    public function relayAdaptiveLoadFactor()
    {
        $diagnostic = Diagnostic::name('Relay Load Factor');

        if ($this->usingRelayAdaptiveCache()) {
            /** @var \RedisCachePro\Connections\RelayConnection $connection */
            $connection = $this->connection;

            return $diagnostic->value(sprintf(
                '%.2f%%',
                $connection->adaptiveCache()->loadFactor()
            ));
        }

        return $diagnostic->value('Unused');
    }
    ...
    /**
     * Returns the `tracking_total_keys` from Redis.
     *
     * @return string|null
     */
    public function trackingTotalKeys()
    {
        return $this->redisInfoKey('tracking_total_keys');
    }

    /**
     * Returns the maximum number of keys in invalidation table.
     *
     * @return string|null
     */
    public function trackingTableMaxKeys()
    {
        return $this->redisConfigKey('tracking-table-max-keys');
    }

    /**
     * Returns the maximum number of connected clients.
     *
     * @return string|null
     */
    public function maxClients()
    {
        return $this->redisConfigKey('maxclients');
    }

    /**
     * Returns details about key tracking.
     *
     * @return \RedisCachePro\Diagnostics\Diagnostic
     */
    protected function keyTracking()
    {
        $diagnostic = Diagnostic::name('Key Tracking');

        if (! $this->connection) {
            return $diagnostic;
        }

        $keys = (int) $this->trackingTotalKeys();
        $maxKeys = (int) $this->trackingTableMaxKeys();

        if ($maxKeys > 0 && $keys > $maxKeys * 0.90) {
            return $diagnostic->warning($keys)->comment('Saturated');
        }

        return $diagnostic->value($keys);
    }
    ...
    protected function phpredis()
    {
        ...
        if (version_compare($version, '5.3', '<')) {
            return $diagnostic->error($version)->comment('Unstable');
        }
    ...
    protected function relay()
    {
        ...
        if (version_compare($version, '0.10.1', '<')) {
            return $diagnostic->error($version)->comment('Unstable');
        }
    ...
    protected function phpVersion()
    {
        ...
        // 8.3 (security support until 8 Dec 2025)
        if (version_compare($version, '8.4', '<') && date('Y') >= 2027) {
            return $diagnostic->warning($version)->comment('Outdated');
        }
    ...
    {
        if (! $this->config) {
            return new WP_Error('configuration_missing', 'Missing configuration.');
        }

        if ($this->isDisabled()) {
            return new WP_Error('cache_disabled', 'Object cache is disabled.');
        }

        try {
            $this->config->connector::connect($this->config)->ping();
        } catch (Throwable $exception) {
            return new WP_Error('ping_failed', $exception->getMessage());
        }
    ...
    public function redisVersion()
    {
        ...
        if ($this->clientIsRelay() && version_compare($version, '6.2.7', '<')) {
            return $diagnostic->error($version)->comment('Unsupported');
        }
    ...
    /**
     * Returns the server type, if a connection is established.
     *
     * @return \RedisCachePro\Diagnostics\Diagnostic
     */
    public function serverType()
    {
        ...
        if ($info = $this->redisInfo()) {
            if (isset($info['valkey_version'])) {
                $type = 'Valkey';
            } elseif (isset($info['dragonfly_version'])) {
                $type = 'DragonflyDB';
            } elseif (stripos((string) ($info['executable'] ?? ''), 'keydb') !== false) {
                $type = 'KeyDB';
            } else {
                $type = 'Redis';
            }
    ...
    {
        if ($this->connection) {
            return $this->connection->memoize('info');
        }
    ...
    protected function redisConfig()
    {
        ...
        if (! $config) {
            $config = $this->connection->config('GET', '*');
        }
    ...
    /**
     * Returns specific key from Redis `CONFIG` call.
     *
     * @param  string  $name
     * @return string|null
     */
    protected function redisConfigKey($name)
    {
    ...
    /**
     * Whether the object cache is configured to use Relay.
     *
     * @return bool
     */
    public function clientIsPhpRedis()
    {
        return ($this->config->clientName ?? '') === 'PhpRedis';
    }
     ...
     * @return bool
     */
    public function clientIsRelay()
    {
        return ($this->config->clientName ?? '') === 'Relay';
    }
    ...
    /**
     * Whether the object cache is using Relay's adaptive cache.
     *
     * @return bool
     */
    public function usingRelayAdaptiveCache()
    {
        if ($this->connection instanceof RelayConnection) {
            if ($adaptiveCache = $this->connection->adaptiveCache()) {
                return property_exists($adaptiveCache, 'status')
                    ? $adaptiveCache->status === 'active'
                    : $adaptiveCache->enabled;
            }
        }
    ...
    public static function host()
    {
        ...
        if (
            ...
            ! empty($_SERVER['WPAAS_V2_SITE_ID']) ||
        ...
        if (strpos($hostname, '.onrocket.com') || strpos($hostname, '.wpedgeserver.com')) {
            return 'rocket';
        }
     ...
     *
     * @return array<string, array<int|string, Diagnostic>>
     */
     ...
     *
     * @param  string  $offset
     * @return array<int|string, Diagnostic>|null
     */
