...
class RelayConnection extends PhpRedisConnection implements ConnectionInterface
{
    ...
    /**
     * Returns the adaptive cache object.
     *
     * Bypasses the `command()` method to avoid log spam.
     *
     * @return \Relay\AdaptiveCache|false
     */
    public function adaptiveCache()
    {
        return $this->client()->adaptiveCache();
    }
    ...
    /**
     * Flushes Relay's in-memory cache.
     *
     * Bypasses the `command()` method to avoid log spam.
     *
    ...
    /**
     * Returns the timestamp of the last in-memory cache flush.
     *
     * Bypasses the `command()` method to avoid log spam.
     *
     * @param  string|null  $endpointId
     ...
     * @return mixed
     */
    public function lastMemoryFlush(?string $endpointId = null, ?int $db = null)
    {
        return $this->client()->lastMemoryFlush($endpointId, $db);
    }
     ...
     *
     * @param  float  $flushRequestedAt
     * @param  int    $database
     * @return void
     */
    public function maybeFlushRelayMemory($flushRequestedAt, $database)
    {
        $endpoint = $this->endpointId() ?: null;
        $flushedAt = $this->lastMemoryFlush($endpoint, $database);

        if ($flushedAt < $flushRequestedAt) {
            $this->flushMemory($endpoint, $database);
        }
    }
}
